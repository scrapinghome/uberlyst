...
class ManageUsersController extends Controller
{
    ...
    public function verify($id)
    {
        ...
        $notify[] = ['success', 'User profile verified successfully!'];
    ...
    {
        $page_title = 'Profile Unverified Users';
        $empty_message = 'No profile unverified user found';
        $users = User::profileUnverified()->latest()->paginate(getPaginate());
        return view('admin.users.list', compact('page_title', 'empty_message', 'users'));
    }    
    ...
    public function detail($id)
    {
        ...
        $totalAd = AdList::where('user_id',$user->id)->where('status',1)->count();
        $totalPromoted = AdPromote::where('user_id',$user->id)->count();
        $totalTransaction = Transaction::where('user_id',$user->id)->count();
        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','totalTransaction','totalAd','totalPromoted'));
    }
    ...
    public function userAds(Request $request,$id)
    {
        ...
        if($search){
            ...
            $ads = AdList::where('user_id',$user->id)->where('title','like',"%$search%")->whereHas('category',function($ad) use($search){
                $ad->where("name",'like',"%$search%");
            })->latest()->paginate(getPaginate());

        } else {
            $page_title = "Ads of $user->username";
            $ads = AdList::where('user_id',$user->id)->latest()->paginate(getPaginate());
        }
        $empty_message = "No ads found";
        return view('admin.ads.allAds',compact('page_title','ads','search','empty_message'));
    }

    public function userPromotedAds(Request $request,$id)
    {
        $search = $request->search;
        $user = User::findOrFail($id);
        if($search){
            $page_title = "Search Result of $search";
            $requests = AdPromote::where('user_id',$user->id)->whereHas('ad',function($ad) use($search){
                $ad->where("title",'like',"%$search%");
            })->latest()->paginate(getPaginate());
        } else {
            $page_title = "Promotions of $user->username";
            $requests = AdPromote::where('user_id',$user->id)->latest()->paginate(getPaginate());
