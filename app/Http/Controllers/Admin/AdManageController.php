...
class AdManageController extends Controller
{
     ...
     public function buyerrequests(Request $request)
    {
        ...
        if($search){
            $page_title = "Search Result of $search";
            $reply = AdList::where('title','like',"%$search%")->whereHas('category',function($ad) use($search){
                $ad->where("name",'like',"%$search%");
            })->latest()->paginate(15);

        } else {*/
            $page_title = "Buyer Replies Requests";
            $replies = DB::table('buyer_requests')->paginate(15);
        
        $empty_message = "No ads found";
        
        return view('admin.buyer_requests',compact('page_title','replies','empty_message'));
