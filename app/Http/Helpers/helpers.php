...
//moveable
function uploadImage($file, $location, $size = null, $old = null, $thumb = null,$watermark = 0)
{
    ...
    try{
        ...
        if($watermark == 1){
            $wtrmark = Image::make('assets/images/watermark.png')->opacity(55)->rotate(0)->greyscale()->fit($image->width(), $image->height());
            $image->insert($wtrmark,'center'); 
        }
...
function sendGeneralEmail($email, $subject, $message, $receiver_name = '')
{
    ...
    if ($config->name == 'php') {
        sendPhpMail($email, $receiver_name, $general->email_from, $message,$subject);
    } else if ($config->name == 'smtp') {
...
function gatewayRedirectUrl($type = false){
    ...
    if($pkg){
        ...
        return  $type ? 'user.history':'user.deposit';
    }
   
}

function gatewayReturnMsg(){
...
function queryBuild($key, $value)
{
    ...
    if(Route::currentRouteName() == 'home'){
        return '/items'.$delimeter."$key=$value";
    }
    if(Route::currentRouteName() != 'ads'){
        return '/all'.$delimeter."$key=$value";
    }
    ...
    function advertisements($size) {
        ...
        if (!empty($ad)) {
            ...
            if ($ad->type == 1) {
                return  '<a class="advert" data-advertid="'.$ad->id.'"  target="_blank" href="'.$ad->redirect_url.'"><img src="'.getImage('assets/images/commercial/'.$ad->ad_image).'" alt="image" class="w-100"></a>';
            }
            if($ad->type == 2) {
