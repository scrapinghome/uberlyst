     ...
     <div class="d-flex justify-content-between title">
      ...
      <label class="mb-3 font-weight-bold">@lang('Item Images')</label>
       ...
       <div class="col-md-12 form-group">
        ...
        @if (($subcategory->one_time || $subcategory->per_month || $subcategory->per_six_month || $subcategory->per_year) && (!$subscription_per_month && !$subscription_per_six_month && !$subscription_per_year))  
        <div class="col-md-12 form-group">
          <label>@lang('Payment Needed. How do you want to pay?')</label>
          <select class="form--control" name="pay_type" required>
            <option value="one_time">One Time ({{$subcategory->one_time ? '$' . $subcategory->one_time : 'Free'}}) for this ad only</option>
            <option value="per_month">Per Month ({{$subcategory->per_month ? '$' . $subcategory->per_month : 'Free'}}) for any ad within next 30 days</option>
            <option value="per_six_month">Per 6 Months ({{$subcategory->per_six_month ? '$' . $subcategory->per_six_month : 'Free'}}) for any ad within next 6 months</option>
            <option value="per_year">Per Year ({{$subcategory->per_year ? '$' . $subcategory->per_year : 'Free'}}) for any ad within next one year</option>
          </select>
        </div>
        @endif       
